...
const Installedapps = () => {
    ...
    const hanndlesort =(type) =>{
        setSort(type)
        if (type==="ascending") {
            const sortByassending =[...applist].sort((a,b)=>a.ratingAvg-b.ratingAvg)
            setapplist(sortByassending)
            
        }
        if (type==="descending") {
            const sortBydssending =[...applist].sort((a,b)=>b.ratingAvg-a.ratingAvg)
            setapplist(sortBydssending)
  ...
  <ul className="menu dropdown-content bg-base-100 rounded-box z-1 w-52 p-2 shadow-sm">
    ...
    <li><a  onClick={()=>hanndlesort("descending")}>descending </a></li>
             ...
             {
                applist.map((app,index)=> <Installapp key={index} app={app}></Installapp>)
            }
           </div>
        </div>
    );
